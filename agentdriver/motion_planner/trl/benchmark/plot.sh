...
python -m openrlbenchmark.rlops_multi_metrics \
    --filters '?we=Anonymous&wpn=trl&xaxis=_step&ceik=trl_ppo_trainer_config.value.reward_model&cen=trl_ppo_trainer_config.value.exp_name&metrics=env/reward_mean&metrics=objective/kl' \
        "sentiment_tuning?tag=$BASELINE_PR_TAG&cl=sentiment anonymous/gpt2-imdb ($BASELINE_PR_NAME)" \
    --env-ids sentiment-analysis:anonymous/distilbert-imdb \
    --no-check-empty-runs \
    --pc.ncols 2 \
    --pc.ncols-legend 1 \
...
python -m openrlbenchmark.rlops_multi_metrics \
    --filters '?we=Anonymous&wpn=trl&xaxis=_step&ceik=trl_ppo_trainer_config.value.reward_model&cen=trl_ppo_trainer_config.value.exp_name&metrics=env/reward_mean&metrics=objective/kl' \
        "sentiment_tuning?tag=$BASELINE_PR_TAG&cl=sentiment anonymous/gpt2-imdb ($BASELINE_PR_NAME)" \
        "sentiment_tuning_step_grad_accu?tag=$BASELINE_PR_TAG&cl=sentiment anonymous/gpt2-imdb gradient accumulation ($BASELINE_PR_NAME)" \
    --env-ids sentiment-analysis:anonymous/distilbert-imdb \
    --no-check-empty-runs \
    --pc.ncols 2 \
    --pc.ncols-legend 1 \
...
python -m openrlbenchmark.rlops_multi_metrics \
    --filters '?we=Anonymous&wpn=trl&xaxis=_step&ceik=trl_ppo_trainer_config.value.reward_model&cen=trl_ppo_trainer_config.value.exp_name&metrics=env/reward_mean&metrics=objective/kl' \
        "sentiment_tuning?tag=$BASELINE_PR_TAG&cl=sentiment anonymous/gpt2-imdb ($BASELINE_PR_NAME)" \
        "sentiment_tuning_peft?tag=$BASELINE_PR_TAG&cl=sentiment anonymous/gpt2-imdb w/ peft ($BASELINE_PR_NAME)" \
    --env-ids sentiment-analysis:anonymous/distilbert-imdb \
    --no-check-empty-runs \
    --pc.ncols 2 \
    --pc.ncols-legend 1 \
